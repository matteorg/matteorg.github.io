...
$$\delta = \frac{\\#\left\\{(x_i,y_j):x_i>y_j\right\\}-\\#\left\\{(x_i,y_j):x_i<y_j\right\\}}{n_1n_2}$$

Imagine you have two groups with the following observations: $T=\left\\{5,3,8\right\\}$ and $C=\left\\{4,6,4\right\\}$.

We would need to compare every observation in group $T$ to every observation in group $C$. This means that we'd have $3x3=9$ comparisons. Specifically, $5>4$, $5>4$, $\color{red}{5<6}$, $\color{red}{3<6}$, $\color{red}{3<4}$, $\color{red}{3<6}$, $8>4$, $8>4$, $8>6$ (in red you see the cases where observations in $T$ are smaller than those in $C$ and white otherwise). Since in 5 cases observations in $T$ are larger than those in $C$ and in 4 they are smaller, we get:

$$\delta=\frac{5-4}{3*3}=0.11$$

Which reads: "Observations in $T$ tend to be larger but magnitude of the difference seems negligible" (see below for suggested reference values).

This formula effectively measures the probability that a randomly chosen value from one group is larger than a randomly chosen value from the other, minus the probability of the opposite. Because it uses rank-based comparisons, Cliff's delta is highly resistant to outliers and skewed data. Its value ranges from $-1$ to $1$ ($1$ means all values in one group are larger than all values in the other - no overlap), where, according to [Meissel and Yao (2024)](https://www.researchgate.net/publication/377696423_Using_Cliff's_Delta_as_a_Non-Parametric_Effect_Size_Measure_An_Accessible_Web_App_and_R_Tutorial):
